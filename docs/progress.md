...
| 날짜 | 결정 내용 | 이유 |
|------|-----------|------|
| 2026-06-15 | [Gotcha] OTA 재게시 시 `feature/diary-record-redesign`(`a6ef16a`, main 대비 다이어리 리디자인 4커밋 선행)으로 게시했던 첫 번째 재게시를 폐기하고, `main`(`6430e10`)으로 체크아웃 후 `eas update --branch production --environment production`으로 다시 게시 (update group `6724c1e7-b7ed-46a7-99a3-41117f1f1958`). **앞으로 production/preview OTA는 반드시 `main` 브랜치 기준으로만 게시한다** (feature 브랜치 기준으로 게시하면 main에 머지되지 않은 미검증 변경사항이 프로덕션에 먼저 노출됨) | EAS Update의 "branch"(배포 채널)는 git 브랜치와 무관하게 `eas update` 실행 시점의 로컬 워킹트리 코드를 그대로 번들링/배포함. 첫 재게시 때 feature 브랜치(다이어리 리디자인 포함)에서 게시해, env 버그 수정과 함께 main에 없는 미검증 UI 변경사항까지 모든 프로덕션 사용자에게 노출됨 → main으로 재-재게시하여 복구 |
| 2026-06-15 | 다이어리 로컬 녹음 기능 추가에 따라 "음성을 녹음하지 않는다"는 기존 안내 문구를 앱/web에서 전부 수정 + `PRIVACY_VERSION` 1.0 → 1.1 | (1) `deci-duel-app/src/screens/auth/MicTestScreen.tsx` 온보딩 개인정보 카드: "녹음 파일은 서버로 전송되지 않아. 측정값(dB)만 기록돼." → "다이어리에 남긴 녹음은 내 폰에만 저장돼. 서버로 전송되지 않고, 다른 사람에게 공개되지 않아." (2) `deci-duel-web/src/pages/StaticPages.tsx` 랜딩 "Privacy First" 섹션 본문 수정(제목 "음성 파일은 서버로 전송되지 않습니다"는 여전히 사실이라 유지). (3) 개인정보처리방침 "다. 마이크(음성) 관련 안내" 섹션을 대결/솔로 측정(dB 수치만 서버 전송)과 다이어리 녹음(본인 기기에만 저장, 서버 미전송·미공유, 다이어리 삭제 시 함께 삭제 — `diaryStore.deleteEntry`의 `deleteRecording` 호출로 확인)으로 분리 서술. 정책 본문의 데이터 처리 방식이 실질적으로 변경되어 `PRIVACY_VERSION`을 1.1로 올림(신규 가입자는 새 버전에 동의; 기존 유저 재동의 플로우는 미구현 상태이므로 별도 과제로 남김) |
...
| 2026-06-13 | 다이어리 무드 이모지를 Fluent Emoji 3D(MIT, jsdelivr CDN)로 교체 + 캘린더 그리드 폰트 확대 | 사용자가 제공한 이모지 스타일 비교(Default/Twemoji/OpenMoji/Fluent 3D)에서 Fluent 3D를 ★추천으로 지정. `src/utils/moodEmojiAssets.ts`에 MOODS 7종 → `https://cdn.jsdelivr.net/gh/microsoft/fluentui-emoji@main/assets/<Title Case Name>/3D/<snake_case>_3d.png` 매핑(전부 HTTP 200 확인), `MoodPicker.tsx`에 `MoodEmoji`(Image + onError 시 네이티브 Text로 fallback) 컴포넌트 신규 추가. `MoodRow` 칩, `CalendarScreen.tsx` 상세 시트 큰 무드 아이콘, `QuickLogSheet.tsx` resultMode 큰 무드 아이콘, **캘린더 월간 그리드 day cell**(`cellEmojiWrap`)까지 전부 `MoodEmoji`로 교체해 일관된 3D 이모지 렌더링 확보. 동시에 다이어리 상세/편집 시트 전반(날짜, dB 값/단위, 코멘트, 시트 라벨) 폰트 크기 확대 및 댓글 입력란/스크롤 높이 제한 완화 |
| 2026-06-13 | `MoodRow`의 "+"(커스텀 이모지 추가) 버튼은 항상 "+" 유지, 커스텀 선택 시 `chipSelected` 핑크 하이라이트만 적용 | 사용자 피드백: 커스텀 이모지를 고르면 "+" 버튼 자리에 그 이모지가 채워지고 "+"가 별도 칸으로 밀려나는 게 아니라, "+" 버튼 자체는 그대로 남아있어야 함. `isCustom`(현재 mood가 MOODS 7종에 없음)일 때만 "+" 칩에 `chipSelected` 스타일을 추가로 적용하도록 단순화 |
| 2026-06-13 | rn-emoji-keyboard 이모지 피커 전체를 Twemoji(jdecked/twemoji fork, jsdelivr CDN)로 패치 + patch-package 적용 | 사용자가 "+" 버튼으로 여는 전체 이모지 피커(수천 개 + 스킨톤)도 동일하게 image 기반 렌더링을 원함. `twemoji-parser`(npm)로 이모지별 정확한 코드포인트(FE0F 포함 여부 등 ZWJ 시퀀스 케이스 포함)를 구해 `https://cdn.jsdelivr.net/gh/jdecked/twemoji@latest/assets/72x72/<codepoints>.png` URL을 생성하는 `TwemojiGlyph` 컴포넌트를 `node_modules/rn-emoji-keyboard/src/components/`에 신규 작성(이미지 로드 실패 시 네이티브 텍스트 emoji로 fallback). `SingleEmoji.tsx`/`SingleSkinTone.tsx`의 `<Text>{emoji}</Text>` 렌더를 `<TwemojiGlyph>`로 교체 (rn-emoji-keyboard는 `"react-native": "src/index"`라 Metro가 컴파일된 lib이 아닌 src를 직접 사용 — node_modules 직접 수정이 번들에 즉시 반영됨). `npx patch-package rn-emoji-keyboard`로 `patches/rn-emoji-keyboard+1.7.0.patch` 생성 + `package.json`에 `"postinstall": "patch-package"` 추가해 `npm install` 후에도 패치 유지. EmojiCategory.tsx/SkinTones.tsx는 추가 패치 불필요 확인(직접 이모지 렌더 없음) |
| 2026-06-13 | [REVERT] 위 두 건(Fluent Emoji 3D, Twemoji rn-emoji-keyboard 패치)을 전부 되돌리고 네이티브 이모지 렌더링으로 복귀 + 캘린더 그리드 이모지/dB 폰트 확대 | 번들에 새 코드가 포함된 것은 grep으로 확인됐으나, 시뮬레이터에서 Fluent 3D/Twemoji 이미지가 전혀 렌더링되지 않고 계속 네이티브 플랫 이모지로 표시됨(원인 미확정 — `onError` fallback 또는 네트워크/ATS 이슈 추정). 사용자가 디버깅 대신 전면 롤백을 지시. `MoodEmoji`를 `<Text style={{fontSize:size, lineHeight:size*1.2}}>{emoji}</Text>`로 단순화하고 `moodEmojiAssets.ts` 삭제. `node_modules/rn-emoji-keyboard/src/components/SingleEmoji.tsx`/`SingleSkinTone.tsx`를 원본(`<Text>{emoji}</Text>`)으로 되돌리고 `TwemojiGlyph.tsx` 삭제, `patches/rn-emoji-keyboard+1.7.0.patch` 삭제, `package.json`의 `postinstall: patch-package` 제거 + `npm uninstall twemoji-parser patch-package`. 동시에 `CalendarScreen.tsx` 월간 그리드 day cell의 무드 이모지를 16px→22px, dB 숫자 폰트를 `FS.xs`(11)→`FS.sm`(13), `cellEmojiWrap.height`를 18→24로 확대(cellHeight≈59px 기준 클리핑 없음 확인). 시뮬레이터에서 캘린더 그리드/상세시트/MoodRow/이모지피커 전부 네이티브 이모지로 정상 렌더 확인, `tsc --noEmit` 통과 |
